# Laboratorio-3
Trabajo de laboratorio 3 para la clase de Programación 1 (program.cs y word)
...
class Program{
    ...
    static void Main(){
        ...
        do{ //ciclo para mostrar el menú de opciones, hecho con do while para que se ejecute al menos una vez
            ...
            switch (opcion){ //switch para seleccionar la opción deseada
    ...
    static void addEstudiante(){ //función para agregar un estudiante
    ...
    static void listEstudiante(){ //función para mostrar la lista de estudiantes
    ...
    static void caliEstudiante(){ //funcion para mostrar el estudiante con la calificación más alta

        if (calificaciones.Count == 0){
            Console.WriteLine("No hay calificaciones registradas.");
        }
        else{ //Busca la calificación más alta en la lista, guarda la calificación y el nombre del estudiante.
